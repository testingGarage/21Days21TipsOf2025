...
# Day 10 Tip - Reflog is the map in your hand for exploring the changes in your local.
...
# Day 11 Tip - Knowing the difference is helpful; it helps in understanding the clarity.
...
# Day 12 Tip - Reading and understanding the outcome of ```git diff``` should be a habit and a skill.
...
# Day 13 - Using ```Gitk``` to know the Git is a visual learning.

In the last three tips I learned about ```reflog``` and ```diff```.  And, in the previous comments I learnned about basic and routine git commands.  All these were done through an interface - Command Line Interface.

Can I have a visual look of all these so that I understand better?

## Browsing the repository with ```Gitk```

To a beginner with Git, Gitk is a useful utility to learn visually.  It is packaged with Git.  You can see the log here, and visually. [^11] [^12] [^13] [^14]

Refer to the reference links 11, 12, 13 and 14.  It explains Gitk's feature and its user interface.

The Gitk can be opened with this command ```gitk```.
...
# Day 14 - Be comfortable to probe more using ```Gitk``` to learn the changes, and help your debugging. 
...
With ```git log```, ```git reflog```, ```git diff``` and ```gitk``` we are in a springboard to jump start the debugging in our woring repo.

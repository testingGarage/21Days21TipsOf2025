...
If you have a GitHub account, more likely you could have also **forked** a repository.  You might also have watched someone's repository have a forked project.  What is this fork?  When should one use it? How is fork and clone are different?
...
- For example, I can **fork** an existing GitHub repository to my GitHub's account.
    - Doing so, we see that is a forked repository.
    - On successful fork, this forked repository is not seen my local machine.
        - Instead, this is seen on cloud space of GitHub's account.
...
You will use **clone** when you have write access and wanting to contribute by collaborating.

You will use **fork** when you want to contribute to a repository for which you do not have write access.  Not that forking is not duplicating (mirroring) the repository.

When you want to mirror a repository, explore on it.  In my so far experience, I have not mirroed any repository as a backup.  But, you should be knowing about it. Explore!

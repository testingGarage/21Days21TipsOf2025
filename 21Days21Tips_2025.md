...
$ git checkout main
$ git merge bug-fix-branch -m "write the custom merge message on what and why are you merging".
```
...
# Day 16's Tip - Use ***fetch*** to download the changes in remote repository to the local repository. It avoids merge conflicts to a considerable extent.

Anytime you see there are changes in remote repository (on GitHub) and that is not seen in your local repository, do not get panicked.  You can update your local references of remote with ```git fetch```.  

The ```git fetch``` updates the local repository without merging them.  This is important to notice.

## Illustration of ```git fetch```

1. The remote repository (on GitHub) as branch ```main```.  But, it is not available in local repository on my machine.  So, I see the error -- ```error: pathspec 'main' did not match any file(s) known to git```

    ![The git fetch main command says there is no reference to branch main in local.](/resources/day16TipPic_branchNotAvailableInLocal_1.png "The 'git fetch main' command says there is no reference to branch main in local.")

2. The local branch not showing the branch ```main```.

    ![The local branch not showing the branch main, while the remote repo has it.](/resources/day16TipPic_localBranchMissingBranchMain_2a.png "The local branch not showing the branch main, while the remote repo has it.")

3. The local branch not showing the remote branch reference ```main```, because, the local do not have detail or information about it.

    ![The local branch not showing the remote branch reference for main.](/resources/day16TipPic_localBranchMissingBranchMain_2b.png "The local branch not showing the remote branch reference for main.")

4. Doing a ```git fetch``` to get the latest available in remote repository to the local repo.

    ![I'm doing a git fetch to get all latest available on remote repo to the local repo.](/resources/day16TipPic_makingFetch_3a.png "I'm doing a git fetch to get all latest available on remote repo to the local repo.")

5. On successful **fetch**, the local repo is updated with reference of remote ***main**.  This says, there is a branch "main" in remote repo and it is not available in local.

    ![The local repo is updated with remote reference for main.](/resources/day16TipPic_postFetch_4a.png "The local repo is updated with remote reference for main.")

6. Note that, post fetch, there is no difference between current local active branch ```Week3Tips``` and remote branch ```Week3Tips```.  

    But, the ```git diff``` shows there is a difference between current local active branch ```Week3Tips``` and remote branch ```main```.  

    ![The differences seen in local branch Week3Tips and remote branch main.](/resources/day16TipPic_postFetch_4a.png "The differences seen in local branch Week3Tips and remote branch main.")


To summarize,
1. The ```git fetch``` pulled all latest changes available in remote to local repo, and it did not merge.  It just pulled.
2. When you want to merge your changes to a remote branch, it is a useful practice to **fetch** latest changes avaialble in remote. and then do merge locally.  This reduces [and avoids] the merge conflicts to considerable extent.


# Day 17's Tip - When merging add a commit message.  It helps you for first when you come back later to know what happened in the merge.

When you are merging the changes of a branch to another, add a message.  This helps you, your team, and aids in debugging when there is a problem in future.
...
$ git merge bug-fix-branch -m "write the custom merge message on what and why are you merging".
```
## Illustration of Merge Commit Message

![Merge of two branches with a commit message.](/resources/day17TipPic_mergeWithCommitMessage_1.png "Merge of two branches with a commit message.")


![The git log showing the merge commit message for reference.](/resources/day17TipPic_mergeWithCommitMessage_2.png "The git log showing the merge commit message for reference.")


![The commit message in GitHub showing the merge commit message.  This helps one to know what was merged.](/resources/day17TipPic_mergeWithCommitMessage_3.png "The commit message in GitHub showing the merge commit message.  This helps one to know what was merged.")


# Day 18's Tip - Know the merge strategy used by you and the team.  Talk to team what merge strategy to use when there is no linear history for branches.

While I'm writing this, the Git on my machine is of version ```2.47.1.windows.2```.

The default merge strategy on this version is **ort**. Prior to this, ```recurssive``` was the default merge strategy.  ort stands for **Ostensibly Recursive's Twin**.

Here are the available merge strategies:
1. recurssive
2. resolve
3. octopus
4. ours
...
Refer to this page for more details on these strategies. [^20]

[^20]: https://git-scm.com/docs/merge-strategies


## Changing the Merge Strategy
To change the merge strategy use the below command.

```git merge -s strategy-name branch-name```

For the strategy-name use the merge strategy you need.
For the brnach-name, use the branch that you want to merge with current active branch.

**Note**: Talk to your team what is the merge strategy used in general and in the context specific to the history of branches invovled.


## Squash Merge

The squash merge takes bunch of commits from a branch and combine them into a single commit before into another branch.

Use the below command for making a squash merge.
```git merge --squash <branch-name>```

### When to use squash merge?
- When you are working with team who is making multiple commits, and you want a clean commit per feature.
    - That is, when you want to keep clean and concise history in the target branch.

## Factors to consider when choosing merge strategy
1. Branch history
2. Do you want to preserve individual commits?
3. Do you want to keep simple, clean and consise history?
4. What guidelines and practice do your team follow for merge?